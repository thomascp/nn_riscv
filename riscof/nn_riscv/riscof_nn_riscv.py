...
class nn_riscv(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","vvp")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       self.objcopy_cmd = 'riscv{0}-unknown-elf-objcopy -O binary {1} {2}'
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          # function
          cmd_1 = self.compile_cmd.format(testentry['isa'].lower(), self.xlen, test, elf, compile_macros)

          cmd_2 = self.objcopy_cmd.format(self.xlen, elf, binary)

          # hexdump binary to commands.
          cmd_3 = "hexdump -v -e '1/4 \"%08x\" \"\\n\"' {0} | sed 'N;s/\(.*\)\\n\(.*\)/\\2\\1/' > {1}".format(binary, ram)
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + " " + self.pluginpath + "/../../nnrv"
          else:
            simcmd = 'echo "NO RUN"'

          # concatenate all commands that need to be executed within a make-target.
          execute = '@cd {0}; {1}; {2}; {3}; {4}'.format(testentry['work_dir'], cmd_1, cmd_2, cmd_3, simcmd)
